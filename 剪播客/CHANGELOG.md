...
## v4.0.5 - 2026-02-19 🎨 审查页面交互增强 + 说话人音量对齐

**用户反馈**：5 项迭代需求。

**Major 改进**：

1. **叠词误判修复** — 卡顿词检测排除正常叠词（妈妈、宝宝、哈哈哈、慢慢地、开开心心等）
   - 新增 REDUPLICATED_WORDS 白名单、AA/AAB/AABB 模式排除
   - 原则：宁可漏检，不可误删正常叠词
   - 更新：`用户习惯/5-卡顿词.md`、`用户习惯/6-句内重复检测.md`

2. **说话人音量对齐** — `cut_audio.py` v4，`--speakers-json` 可选参数
   - 用 ffmpeg `aselect` + `volumedetect` 检测各说话人平均响度
   - 以最响说话人为基准，补偿其他人（最大 +6dB，<0.5dB 忽略）
   - 每 speaker 采样最多 30 段，一次 ffmpeg 调用完成

3. **Delete/Backspace 快捷键** — 选中文字后按 Delete 或 Backspace 直接标记删除
   - 等同于点击浮动工具栏的"标记删除"按钮

**Minor 改进**：

4. **说话人修正** — 点击说话人名字弹出下拉菜单
   - 显示所有已知说话人 + 自定义输入
   - 修改自动保存到 localStorage

5. **导出按钮分离** — "导出剪辑文件"(绿色)留在顶部工具栏，"导出AI反馈"(蓝色)移到统计区域
   - 减少用户混淆

**文件变更**：
- `scripts/cut_audio.py` — v4: 说话人音量对齐
- `templates/review_enhanced.html` — Delete 快捷键、说话人修正、按钮分离
- `scripts/generate_review_enhanced.js` — `__SPEAKER_CLASS_FUNC__` 改为 `replaceAll`
- `用户习惯/5-卡顿词.md` — 叠词排除规则
- `用户习惯/6-句内重复检测.md` — 叠词排除

---

## v4.0.3 - 2026-02-18 🎵 自适应淡入淡出消除断句感

**用户反馈**：剪辑后中间会出现明显断句，听感生硬。

**解决**：cut_audio.py v3 — 每个切点自动加淡出/淡入：
- ✅ 自适应时长：`fade = clamp(片段时长 × 8%, 0.03s, 0.3s)`
- ✅ 首段不加淡入，末段不加淡出（自然边界）
- ✅ 极短片段 (< 0.3s) 跳过 fade（避免失真）
- ✅ 安全上限：淡入+淡出总长不超过片段时长的 60%
- ✅ 用 ffmpeg `afade` 滤镜，WAV 级无损处理
...
## v4.0.2 - 2026-02-18 💾 审查页面自动保存

**用户反馈**：审查页面刷新后编辑全部丢失。

**解决**：
- ✅ localStorage 自动保存：每次编辑后 500ms 自动保存
- ✅ 刷新页面自动恢复：删除标记、精剪开关、手动编辑、AI 遗漏反馈
- ✅ 按页面标题区分不同播客，30 天自动清理
- ✅ 关闭页面前强制保存（beforeunload）

**文件变更**：
- `templates/review_enhanced.html` — 新增 autoSave / restoreState
