...
3. **（可选）过渡音乐文件路径**（用于章节间过渡）
   - 短 jingle/stinger，播放在话题切换处
   - 如果不需要过渡音乐，可跳过
   - 如果和片头相同，可以说"同片头"
...
| 过渡 | transition.mp3 | 3秒 | 0.5秒 | 0.5秒 |

是否需要调整时长？直接回复确认，或告诉我你想要的时长（如"片头20秒，片尾10秒，过渡2秒"）。
```

> 如果用户未提供过渡音乐，表格中不显示过渡行。
...
# 过渡音乐（可选，可能与片头相同）
if [ -n "$USER_TRANSITION_MUSIC" ]; then
  cp "$USER_TRANSITION_MUSIC" "$WORK_DIR/theme_transition.mp3"
  THEME_TRANSITION="$WORK_DIR/theme_transition.mp3"
fi
```

**之后所有命令都用 `$WORK_DIR/theme_intro.mp3`、`$WORK_DIR/theme_outro.mp3`、`$WORK_DIR/theme_transition.mp3`，禁止使用用户原始的绝对路径。**
...
| 过渡 | 3秒 | 章节切换处 crossfade 插入 | 时长、音乐文件、插入位置 |

**片头、片尾、过渡可以使用不同的音乐文件**，也可以使用同一首。时长可在启动时自定义。过渡音乐为可选功能，不提供则跳过。
...
```
0. 询问用户：片头音乐、片尾音乐、过渡音乐（可选）、播客音频路径、逐字稿路径
    ↓
0.5 展示默认时长方案（片头15秒、片尾15秒、过渡3秒），等待用户确认或修改
    ↓
1. 分析内容，推荐高亮片段
    ↓
【用户选择 3-4 个片段】
...
5. 分析话题结构，生成时间戳章节
    ↓
5.5 （可选）在章节切换处插入过渡音乐（默认3秒，crossfade）
    ↓
6. 生成标题建议
    ↓
7. 生成播客简介
...
**最常见的错误是只截取"点睛之笔"而丢掉铺垫，导致听众听不懂这句话好在哪里。**

一个好的高亮片段 = **铺垫 + 核心表达 + 落点**，是一个能独立成立的完整语义单元。

| 层次 | 作用 | 示例 |
|------|------|------|
| 铺垫（Setup） | 建立语境，让听众理解接下来为什么重要 | "我在大厂待了五年，每天加班到十一点，有天突然想——" |
| 核心表达（Punch） | 金句本体、观点碰撞、情绪爆发 | "工作的意义不是找到的，是创造的" |
| 落点（Landing） | 反应、停顿、回应，让观点沉淀 | [2秒停顿] "嗯……" / [笑声] / "对，就是这样" |

**❌ 常见信息丢失模式：**

| 模式 | 问题 | 正确做法 |
|------|------|----------|
| 只截金句本身 | "工作的意义是创造的" 脱离语境像鸡汤 | 往前找到铺垫起点（通常是话题转折或提问句） |
| 从句子中间开始 | "...觉得不对劲，然后我就..." 听众不知道谁觉得不对劲 | 回退到说话人开始发言的完整句首 |
| 在反应前截断 | 金句说完就切，没有其他人的反应 | 保留 1-3 秒落点（笑声/感叹/短回应） |
| 多人对话只截一方 | 观点碰撞只保留一个人的话 | 包含对话双方的完整交锋回合 |
| 中途换说话人被切断 | A 说一半 B 插嘴，截取只有 A 的半句 | 回退到 A 的完整句起点，或延伸到 B 回应结束 |

### 什么是好的高亮片段

| 特征 | 说明 | 完整性要求 |
|------|------|------------|
| 金句 | 有力、简洁、有记忆点的表达 | 必须包含引出这句话的语境（为什么说这句） |
| 观点碰撞 | 不同意见的精彩交锋 | 必须包含双方各至少一轮完整发言 |
| 情绪高点 | 笑声、感叹、惊讶的时刻 | 必须包含触发情绪的内容 + 情绪反应本身 |
| 悬念/钩子 | 让听众想继续听下去的内容 | 铺垫 + 悬念句，不需要揭晓答案 |
| 核心洞察 | 播客最有价值的观点浓缩 | 包含推理过程，不只是结论 |
| 故事片段 | 生动的叙事段落 | 完整的故事弧：起因 → 经过（至少部分）→ 金句/反转 |

### 识别流程（5 步）

#### Step 1: 通读转录，标记候选锚点

先快速标记"核心表达"所在位置（锚点），不急着定边界。

寻找的信号：
- **修辞力量**：比喻、反问、排比、对比、类比
- **情绪爆发**：笑声、感叹词、语气突变、停顿后的重启
- **认知转折**：但是、其实、说实话、后来我发现、直到有一天
- **总结性表达**：所以我觉得、归根结底、说到底、核心就是
- **故事高潮**：然后就、结果、没想到、当时我就
...
| 起点信号 | 说明 |
|----------|------|
| 话题转折 | "说到X"、"其实我一直在想" — 新论点的开始 |
| 提问句 | "你有没有想过..."、"为什么会这样呢" — 自然的引入 |
| 说话人切换 | 上一个人说完 → 当前人开始回应 — 回应的起点 |
| 叙事开头 | "那是XX年"、"有一次"、"我记得" — 故事的起点 |
| 段落首句 | 停顿 > 1 秒后的第一句话 — 新段落 |

**⚠️ 向前扩展不超过 3 句**。如果 3 句仍找不到自然起点，说明这个锚点可能需要更多上下文，考虑放弃或接受信息不完整。
...
| 落点信号 | 说明 |
|----------|------|
| 听众/对方反应 | [笑]、"对对对"、"哇"、"真的假的" — 保留 1-3 秒 |
| 自然停顿 | > 1 秒的静音 — 论点沉淀的标志 |
| 话题切换 | 开始聊新话题 — 在此之前截断 |
| 说话人总结 | "就是这样"、"嗯" — 收束语 |

**⚠️ 向后扩展不超过 2 句/5 秒**。落点过长会拖沓。
...
```
□ 独立可懂：一个没听过这期播客的人，能否理解这个片段在说什么？
□ 无悬空代词：片段开头不以"他"、"那个"、"这件事"等指代不明的词开始
□ 无半句话：起点和终点都是完整的句子，不在句中截断
□ 有情绪弧线：听完有感觉（好笑/震撼/好奇/认同），而非平淡无感
□ 时长合理：10-30 秒，铺垫不超过片段的 40%
```
...
- **起点**：对齐到句子开头的第一个字（不要从句中间开始）
- **终点**：在落点反应之后、下一句开始之前
- **时间精度**：用 `subtitles_words.json` 的词级时间戳精确定位，不要凭大概印象
- **留 0.3 秒 buffer**：起点提前 0.3 秒、终点延后 0.3 秒，避免截掉首字/尾字
...
### 片段 1 ⭐⭐⭐ | 15:23 - 15:58 (35s) | 响歌歌 | 金句
> **铺垫**："我在大厂待了五年，每天加班到十一点，有天突然想——"
> **核心**："工作的意义不是找到的，是创造的。你不去创造，它就永远不存在。"
> **落点**：[2秒停顿] Maia："嗯……这句话真的hit到我了。"

### 片段 2 ⭐⭐⭐ | 32:38 - 33:12 (34s) | Maia + 安安 | 观点碰撞
> **铺垫**：Maia："我觉得工资就是最大的意义——"
> **核心**：安安："这完全是两回事，钱能买到安全感，但买不到你早上想起床的理由。"
> **落点**：Maia [笑]："好吧你赢了。"

### 片段 3 ⭐⭐ | 48:15 - 48:45 (30s) | 安安 | 故事
> **铺垫**："面试完Google，拿到offer那一刻——"
> **核心**："我坐在车里哭了半小时，不是因为高兴，是因为我终于可以不为签证活着了。"
> **落点**：[沉默3秒]

### 片段 4 ⭐⭐ | 1:05:28 - 1:06:02 (34s) | 响歌歌 + Maia | 情绪高点
> **铺垫**：响歌歌："你知道什么最讽刺吗？"
> **核心**："我在大厂做的最有意义的事，是帮同事搬家。" [全场笑]
> **落点**：Maia："你这个比喻太绝了——等等这不是比喻对吧？" [笑]

### 片段 5 ⭐⭐ | 1:18:15 - 1:18:50 (35s) | Maia | 引发思考
> **铺垫**："录到最后我在想一个问题——"
> **核心**："如果重新选择，我还会选同一条路吗？我觉得会，但理由完全不一样了。"
> **落点**：[停顿] 响歌歌："这个结尾太好了。"
...
请选择 3-4 个，如：`1, 2, 4` 或 `选 1 2 4`
```

### 推荐数量与时长参考

| 播客总时长 | 推荐候选数 | 用户选择数 | 单片段参考时长 |
|------------|-----------|-----------|---------------|
| < 30min | 4-5 个 | 2-3 个 | 15-25 秒 |
| 30min-1h | 5-7 个 | 3-4 个 | 15-30 秒 |
| > 1h | 7-10 个 | 3-4 个 | 20-35 秒 |
...
|------|------|
| 数量 | 3-4 个片段 |
| 单片段时长 | 15-35 秒（含铺垫 + 核心 + 落点） |
| 总时长 | 45-120 秒 |
| 顺序 | 按精彩程度排列，最吸引人的放第一个 |

### FFmpeg 片段提取

**⚠️ 必须用 `atrim` 滤镜，禁止用 `-ss`/`-to`/`-c copy`（对 MP3 截取不准确，见七.1）。**

```bash
# 1. 提取各片段（用 atrim 精确截取 + 首尾微淡化防爆破）
# 片段 1: 15:23 - 15:58 (923s - 958s)
ffmpeg -i podcast.mp3 \
  -af "atrim=start=923:end=958,asetpts=PTS-STARTPTS,afade=t=in:d=0.03,afade=t=out:st=34.7:d=0.05" \
  -c:a libmp3lame -q:a 2 clip1.mp3

# 片段 2: 32:38 - 33:12 (1958s - 1992s)
ffmpeg -i podcast.mp3 \
  -af "atrim=start=1958:end=1992,asetpts=PTS-STARTPTS,afade=t=in:d=0.03,afade=t=out:st=33.7:d=0.05" \
  -c:a libmp3lame -q:a 2 clip2.mp3

# 2. 截取后必须检查音量（-91dB 基本是静音）
ffmpeg -i clip1.mp3 -af "volumedetect" -f null - 2>&1 | grep max_volume
# 正常应在 -20dB 到 0dB 之间

# 3. 截取后试听确认：铺垫句的第一个字是否完整、落点是否保留
```

> **时间精度**：从 `subtitles_words.json` 读取词级时间戳计算精确的 start/end 秒数。起点提前 0.3 秒、终点延后 0.3 秒作为 buffer。
...
在章节切换处插入短音乐 jingle，让话题转换更自然。**仅在用户提供了过渡音乐文件时执行。**
...
| 位置 | 章节切换点（由时间戳章节识别） |
| 时长 | 默认 3 秒，可配 1-5 秒 |
| 过渡方式 | 用 `acrossfade` 与前后内容交叉淡入淡出，不硬切 |
| 音量 | 与片头音乐一致，或由用户指定（默认 100%） |
| 选择性插入 | 只在**主要话题切换**处插入，不在每个章节都加（避免过于碎片化） |

### 时长配置

| 参数 | 默认值 | 可选范围 | 说明 |
|------|--------|----------|------|
| 过渡时长 | 3秒 | 1-5秒 | 过渡音乐播放总时长 |
| crossfade | 0.5秒 | 0.3-1秒 | 与前后内容的交叉淡入淡出 |
| 音量 | 1.0 | 0.3-1.5 | 过渡音乐音量倍数 |
...
| 需要 | 不需要 |
|------|--------|
| 话题 A → 话题 B（主题明显切换） | 同一话题内的小节 |
| 闲聊 → 正题 | 说话人轮换（同一话题） |
| 正片结尾 → 总结回顾 | 章节间已有自然停顿 > 2秒 |

**规则**：生成时间戳章节后，标记其中的**主要话题切换点**（通常 3-6 个），只在这些位置插入过渡音乐。展示给用户确认。
...
THEME_TRANSITION="$WORK_DIR/theme_transition.mp3"
TRANS_DUR=3           # 过渡音乐时长
CROSSFADE=0.5         # crossfade 时长
TRANS_VOL=1.0         # 过渡音乐音量

# 1. 截取过渡音乐片段（用 atrim，不用 -ss）
ffmpeg -i "$THEME_TRANSITION" \
  -af "atrim=start=0:end=$TRANS_DUR,asetpts=PTS-STARTPTS,volume=$TRANS_VOL,afade=t=in:d=0.3,afade=t=out:st=$((TRANS_DUR-0.3)):d=0.3" \
  -c:a pcm_s16le -ar 44100 "$WORK_DIR/transition_clip.wav"

# 2. 在切换点分割正片
# 假设切换点在 478s（07:58）
SPLIT_TIME=478
ffmpeg -i main_content.wav -t $SPLIT_TIME -c:a pcm_s16le part_before.wav
ffmpeg -i main_content.wav -ss $SPLIT_TIME -c:a pcm_s16le part_after.wav

# 3. crossfade 拼接：前段 → 过渡音乐 → 后段
# 先 crossfade 前段和过渡音乐
ffmpeg -i part_before.wav -i transition_clip.wav \
  -filter_complex "acrossfade=d=${CROSSFADE}:c1=tri:c2=tri" \
  -c:a pcm_s16le before_with_trans.wav

# 再 crossfade 结果和后段
ffmpeg -i before_with_trans.wav -i part_after.wav \
  -filter_complex "acrossfade=d=${CROSSFADE}:c1=tri:c2=tri" \
  -c:a pcm_s16le merged_with_trans.wav
```

> **多个切换点**：从最后一个切换点开始向前处理，避免前面插入后时间偏移影响后续切割。

### ⚠️ 注意事项

- **过渡音乐截取位置**：每次插入可以用不同的起点（如 0s, 3s, 6s...），避免每次听到同一段
- **时间戳偏移**：插入过渡音乐后，后续章节的时间戳需要加上偏移（每插入一段加 `TRANS_DUR - CROSSFADE` 秒）
- **先处理过渡，再算最终时间戳**：时间戳章节应在插入过渡音乐之后重新计算
- **不要在片头预览区和片尾区插入**：过渡音乐只用于正片内容的章节切换
...
```
- [ ] 询问用户：片头音乐、片尾音乐、过渡音乐（可选）、播客音频路径、逐字稿路径
- [ ] 展示默认时长方案（片头15秒、片尾15秒、过渡3秒），等待用户确认或修改
- [ ] 分析内容，按5步流程（锚点→铺垫→落点→验证完整性→精调边界）推荐高亮片段
- [ ] 展示完整文本（铺垫+核心+落点），等待用户选择 3-4 个
- [ ] 用 atrim 精确提取片段（词级时间戳 + 0.3s buffer），检查音量，拼接片头预览
- [ ] 添加片头音乐（用户确认的时长，渐入渐出）
- [ ] 添加片尾音乐（用户确认的时长，渐入渐出）
- [ ] 生成时间戳章节
- [ ] （如有过渡音乐）标记主要话题切换点，展示确认，插入过渡音乐
- [ ] 重新计算时间戳偏移
- [ ] 生成标题建议
- [ ] 生成播客简介
```
...
theme_transition.mp3          # 过渡音乐（本地副本，可选）
...
用户: 片头用 ~/Music/intro-jingle.mp3，片尾用 ~/Music/outro-piano.mp3，过渡音乐同片头，播客是 ~/podcast/episode1_v2.mp3
...
AI: 收到！音乐配置如下：
    ...
    | 过渡 | intro-jingle.mp3 | 3秒 | 0.5秒 | 0.5秒 |

    是否需要调整时长？直接回复确认，或告诉我你想要的时长。

用户: 片头20秒，片尾10秒，过渡不变

AI: 好的，已更新：片头20秒（渐入2秒，渐出3秒），片尾10秒（渐入2秒，渐出2秒），过渡3秒不变。

    正在分析内容，寻找高亮片段...

    ## 推荐高亮片段

    ### 片段 1 ⭐⭐⭐ | 15:23 - 15:58 (35s) | 主播A | 金句
    > **铺垫**："我在大厂待了五年，每天加班到十一点，有天突然想——"
    > **核心**："工作的意义不是找到的，是创造的。你不去创造，它就永远不存在。"
    > **落点**：[2秒停顿] 主播B："嗯……这句话真的hit到我了。"

    ### 片段 2 ⭐⭐⭐ | 32:38 - 33:12 (34s) | 主播B + 嘉宾 | 观点碰撞
    > **铺垫**：主播B："我觉得工资就是最大的意义——"
    > **核心**：嘉宾："这完全是两回事，钱能买到安全感，但买不到你早上想起床的理由。"
    > **落点**：主播B [笑]："好吧你赢了。"

    ### 片段 3 ⭐⭐ | 48:15 - 48:45 (30s) | 嘉宾 | 故事
    > **铺垫**："面试完，拿到offer那一刻——"
    > **核心**："我坐在车里哭了半小时，不是因为高兴，是因为我终于可以不为签证活着了。"
    > **落点**：[沉默3秒]

    ### 片段 4 ⭐⭐ | 1:05:28 - 1:06:02 (34s) | 主播A + 主播B | 情绪高点
    > **铺垫**：主播A："你知道什么最讽刺吗？"
    > **核心**："我在大厂做的最有意义的事，是帮同事搬家。" [全场笑]
    > **落点**：主播B："这不是比喻对吧？" [笑]

    ### 片段 5 ⭐⭐ | 1:18:15 - 1:18:50 (35s) | 主播B | 引发思考
    > **铺垫**："录到最后我在想一个问题——"
    > **核心**："如果重新选择，我还会选同一条路吗？我觉得会，但理由完全不一样了。"
    > **落点**：[停顿] 主播A："这个结尾太好了。"
...
### 2026-03-21
- **高亮片段提炼全面重写**：截取经常丢失信息（缺铺垫、断句中、无落点）
  - 新增"完整论点 > 单独金句"核心原则
  - 新增 5 步识别流程：锚点 → 向前扩展找铺垫 → 向后扩展找落点 → 验证完整性 → 精调边界
  - 新增完整性检查清单（独立可懂、无悬空代词、无半句话、有情绪弧线）
  - 推荐格式改为展示完整文本（铺垫+核心+落点），用户可直接判断截取质量
  - FFmpeg 片段提取改用 atrim + 词级时间戳 + 0.3s buffer，替代不准确的 -ss/-to
  - 同步更新示例对话

### 2026-01-17
- **高亮片段不要只是单句**：用户反馈高亮片段可以是几句连起来的一段话，不用只选一句
  - 已更新：推荐时考虑完整段落，包含上下文，让片段更有故事感和完整性
  - → 已在 2026-03-21 的全面重写中系统性解决
